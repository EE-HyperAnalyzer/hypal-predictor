...
def main():
    ...
    dbag = Databag2()
    last_temp = data.iloc[0]["Moscow:Sheremetievo:T"]
    for timestamp, row in tqdm(data.iterrows()):
        dbag.add(
            SensorData(
                ...
                candle=Candle_OHLC(
                    open=last_temp,
                    high=row["Moscow:Sheremetievo:T"],
                    low=row["Moscow:Sheremetievo:T"],
                    close=row["Moscow:Sheremetievo:T"],
                ),
            )
        )
        last_temp = row["Moscow:Sheremetievo:T"]
