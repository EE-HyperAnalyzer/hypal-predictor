...
DATABAG_PATH = EXPERIMENT_PATH / "databags" / "day_2_extended.dbag2"
...
"""
Сценарий работы с датчиком гироскопа
"""
CRITICAL_ZONE = ZoneRule_NOT(
    rule=ZoneRule_AND(
        lhs=ZoneRule_GREATER(value=-3),
        rhs=ZoneRule_LESS(value=+3),
    )
)
...
def main():
    ...
    axis_data = databag.get_data("NightTokyo", "sgp30", "eco2")
    assert axis_data
